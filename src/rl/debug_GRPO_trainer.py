...
#get config file
with open("/home/user/planning-with-llms/src/rl/debug_config.yaml", "r") as f:
...
def get_train_args(max_prompt_length):
    training_args=GRPOConfig(
        ...
        bf16=bool(cfg['training']['lr_scheduler_type']), #OG: not there
...
def train(train_data,model=model):
    ...
    print(f"DEBUG for output mismatch: {len(train_data[0]['input_ids'])}")
    trainer = GRPOTrainer(
        model = model,
        reward_funcs=[
            format_reward,
...
def main(n,split):
    ...
    #debug 
    print(f"Loaded data print {data}")
    ...
    GRPO_trainer=train(train_data=data)

    # GRPO_trainer=train(train_data=data)
    # return data,GRPO_trainer
...
def debug():
    data,GRPO_trainer=main(n=3,split='train')
    sample = data[0]
    input_ids = torch.tensor(sample['input_ids']).unsqueeze(0).to('cuda') 
    print(f"INPUT IDS IN DEBUG: {input_ids}")
    print(f"Data is: {len(data[0]['input_ids'])}")
    input_len=len(data[0]['input_ids'])
    with torch.inference_mode():
        output=GRPO_trainer.model.generate(
        input_ids,
        max_new_tokens=1024,
        temperature=0.8)
    generation=output[0][input_len:]
    decoded = GRPO_utils.processor.decode(generation, skip_special_tokens=True)
