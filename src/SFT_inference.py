...
base_dir='/srv/chawak/planning-with-llms/results/SFT/3_blocks/'
...
def main(df,iterations):
    ...
    for model_it in range(1,iterations):
        ...
        #load pre-trained model for iteration it 
...
response=main(df=eval_data,iterations=iterations)
metrics=response[0]
c_rate=response[1]
...
for i, df in enumerate(metrics):
    if df.empty:
        ...
        print(df)
        path=f'../results/SFT/{n}_blocks/{split}/noMaxToken_tags/iteration_{iterations-1}'
        df.to_csv(path)
        print(f'Metrics saved to {path}')
print(f'Correction rates are: {c_rate}')